...
{	
	...
	for (i = 0; i < o_n_in; i++)
		if (o_in[i])
		{
			if (o_in[i]->status != obj_status_toread && !(o_in[i]->status & obj_status_reading))
				return	0;
			if((o_in[i]->status & obj_status_reading) && (o_in[i]->n_readers == o_in[i]->n_reads))
				return 0;
	...
	for (i = 0; i < o_n_out; i++)
		if (o_out[i] && o_out[i]->status != obj_status_towrite && o_out[i]->status != obj_status_toalloc && !(o_in[i]->status & obj_status_writing))
		{
	...
	for (i = 0; i < o_n_out; i++)
		if (o_out[i])
		{
			if((o_in[i]->status & obj_status_writing) && (o_in[i]->status & obj_status_mask != o_id))
				return 0;
			if (o_out[i]->status == obj_status_toalloc)
			{
				...
				o_out[i]->status = obj_status_writing + o_id;
			}
		}
...
static inline	void	ddq_postrun(ddq_op op, flag_t ret)
{
	if(ret == task_ret_again)
		return;
	...
	for (i = 0; i < o_n_in; i++)
		if (o_in[i])
		{
			if (o_in[i]->prop & obj_prop_consumable)
			{
				...
				if ((o_in[i]->n_readers == o_in[i]->n_reads) && (o_in[i]->status == obj_status_reading))
					o_in[i]->status = obj_status_towrite;
	...
	for (i = 0; i < o_n_out; i++)
		if (o_out[i])
		{
			...
			if(ret == task_ret_write_partial){
				o_in[i]->status = obj_status_writing + o_id;
			}else{
		...
		if (o_in[i])
		{
			if(!(o_in[i]->prop & obj_prop_read_contest)){
				...
				if (o_in[i]->prop & obj_prop_consumable && o_in[i]->version != op->ver_inputs[i])
					o_in[i]->n_reads --;
				if (o_in[i]->prop & obj_prop_consumable && o_in[i]->n_readers == o_in[i]->n_reads && (o_in[i]->status == obj_status_reading))
					o_in[i]->status = obj_status_towrite;
			}
		}
	for (i = 0; i < o_n_out; i++)
