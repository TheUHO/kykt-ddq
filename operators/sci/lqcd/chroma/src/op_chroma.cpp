...
*/
task_ret op_ChromaInitialize0(void *input[4], void **output)
{
...
task_ret op_ChromaInitialize(void **input, void **output)
{
	int	lattdims[4], argc;
	char **	argv;

	lattdims[0] = *(int *) input[0];
	lattdims[1] = *(int *) input[1];
	lattdims[2] = *(int *) input[2];
	lattdims[3] = *(int *) input[3];
	argc = *(int *) input[4]+1;
	argv = (char **) (input+4);	// FIXME: 这里类型转换有没有问题？

	log_printf("op_ChromaInitialize_RNG: argc=%d, lattice=[%d %d %d %d]\n", argc, lattdims[0], lattdims[1], lattdims[2], lattdims[3]);
	for (int i=1; i<argc; i++)
		log_printf("argv[%d] = '%s', \t", i, argv[i]);
	log_printf("\n");

	Chroma::initialize(&argc, &argv);	// FIXME
	multi1d<int> nrow(lattdims, 4);
	Layout::setLattSize(nrow);
	Layout::create();
...
*/
task_ret op_InitRNG0(void **input, void **output)
{
...
{
	char	buf[1024];		// FIXME: 随便定了个最大长度，可以优化
	const char *xml_rng =
		"<RNG>"
		" <Seed>"
		"  <elem>%d</elem>"
		...
		" </Seed>"
		"</RNG>";
	DEF_VAL(int, seed0, input[0]);
	DEF_VAL(int, seed1, input[1]);
	DEF_VAL(int, seed2, input[2]);
	DEF_VAL(int, seed3, input[3]);

	sprintf(buf, xml_rng, seed0, seed1, seed2, seed3);

	GroupXML_t	param_rng;
	param_rng.id = "RNG";
	param_rng.path = "/RNG";
	param_rng.xml = buf;
	log_printf("op_InitRNG: %s %s %s\n", param_rng.id.c_str(), param_rng.path.c_str(), param_rng.xml.c_str());

	InitRNG(param_rng, Chroma::getXMLOutputInstance());
...
input:
  ...
  const GroupXML_t *params_prop_act,
  const GroupXML_t *params_prop_inv,
  const int *t0,
  const int *j_decay,
output:
...
task_ret op_DoInlinePropagatorInverter(void **input, void **output)
{
    ...
    DEF_REF(GroupXML_t, params_prop_act, input[3]);
    DEF_REF(GroupXML_t, params_prop_inv, input[4]);
    DEF_VAL(int, t0, input[5]);
    DEF_VAL(int, j_decay, input[6]);
    XMLFileWriter &xml_out = Chroma::getXMLOutputInstance();

    ChromaProp_t *params_prop = new ChromaProp_t;
    params_prop->fermact.id = params_prop_act.id;
    params_prop->fermact.path = params_prop_act.path;
    params_prop->fermact.xml = params_prop_act.xml;
    params_prop->invParam.id = params_prop_inv.id;
    params_prop->invParam.path = params_prop_inv.path;
    params_prop->invParam.xml = params_prop_inv.xml;
    params_prop->quarkSpinType = QUARK_SPIN_TYPE_FULL;
    params_prop->obsvP = (bool) 0;
...
#define case_type(T)                                                                               \
    case e##T: {                                                                                   \
        DEF_REF(T, quark_prop_source, input[0]);                                                   \
...
task_ret op_ComputeMesons2(void **input, void **output)
{
	...
	DEF_PTR(obj_mem_t, phases, input[2]);
	DEF_VAL(int, gamma_value, input[3]);
	...
	ComputeMesons2(quark_prop_1, quark_prop_2, *(SftMom *)phases->p, t0, gamma_value, mesprop_vec, xml_out);
	return task_ret_ok;
}
...
task_ret op_GroupXML(void **input, void **output)
{
	GroupXML_t *gxml = (GroupXML_t *) output[0];
	gxml->id = AS_CPTR(char, input[0]);
	gxml->path = AS_CPTR(char, input[1]);
	//gxml->xml = AS_CPTR(char, input[2]);
	gxml->xml = AS_CPTR(char, ((obj_mem)input[2])->p);
...
task_ret op_SftMom_Simple(void **input, void **output)
{
	DEF_VAL(int, mom2_max, input[0]);
	DEF_VAL(int, avg_equiv_mom, input[1]);
	DEF_PTR(obj_mem_t, res, output[0]);

	int	j_decay = 3;

	res->p = new SftMom(mom2_max, (bool) avg_equiv_mom, j_decay);
