...
if(!o_sv(pool)){
	o_sv(pool) = thread_pool_init();
	if(!o_sv(pool)){
		ddq_error("processor_pthread : thread_pool_init() fails.\n");
	}
}
...
ddq_waitfor(thread_pool_submit(o_sv(pool), ddq_p) != -1);
