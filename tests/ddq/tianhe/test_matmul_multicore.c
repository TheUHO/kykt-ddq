...
ddq_ring matmul_ring(obj_mem A, obj_mem B, obj_mem C, int_t row_size, int_t col_size,int_t reduce_size, int_t Ma, int_t Ms,int_t Mg, int_t Kg,int_t Ka,int_t Ng,int_t Na,int_t M, int_t N, int_t K,
   ...
   gen_array_Cgsm[6] = 0;
    ...
    // obj obj_size_array = obj_copy(res, size_array, sizeof(int_t) * 5, load_tianhe("raw_delete"), 0);//TODO
    // obj obj_gen_array_A = obj_copy(res, gen_array_A, sizeof(int_t) * (2 * 3 + 1), load_tianhe("raw_delete"), 0);
    // obj obj_gen_array_B = obj_copy(res, gen_array_B, sizeof(int_t) * (2 * 3 + 1), load_tianhe("raw_delete"), 0);
    // obj obj_gen_array_C = obj_copy(res, gen_array_C, sizeof(int_t) * (2 * 3 + 1), load_tianhe("raw_delete"), 0);
    // obj obj_gen_array_Agsm_1 = obj_copy(res, gen_array_Agsm_1, sizeof(int_t) * (2 * 3 + 1), load_tianhe("raw_delete"), 0);
    // obj obj_gen_array_Agsm_2 = obj_copy(res, gen_array_Agsm_2, sizeof(int_t) * (2 * 3 + 1), load_tianhe("raw_delete"), 0);
    // obj obj_gen_array_Bgsm = obj_copy(res, gen_array_Bgsm, sizeof(int_t) * (2 * 3 + 1), load_tianhe("raw_delete"), 0);
    // obj obj_gen_array_Cgsm = obj_copy(res, gen_array_Cgsm, sizeof(int_t) * (2 * 3 + 1), load_tianhe("raw_delete"), 0);

    // obj obj_params_A2Agsm = obj_copy(res, params_A2Agsm, sizeof(int_t) * 6, load_tianhe("raw_delete"), 0);
    // obj obj_params_B2Bam = obj_copy(res, params_B2Bam, sizeof(int_t) * 6, load_tianhe("raw_delete"), 0);
    // obj obj_params_C2Cam = obj_copy(res, params_C2Cam, sizeof(int_t) * 6, load_tianhe("raw_delete"), 0);
    // obj obj_params_gsm2Asm = obj_copy(res, params_gsm2Asm, sizeof(int_t) * 6, load_tianhe("raw_delete"), 0);
    // obj obj_params_Cam2C = obj_copy(res, params_Cam2C, sizeof(int_t) * 6, load_tianhe("raw_delete"), 0);
    ...
    *iter_start = 0;
    ...
    obj obj_iter_start_C = obj_new(res, load_tianhe("long_new"), load_tianhe("raw_delete"), obj_prop_consumable | obj_prop_read_contest | (reduce_size/Kg));

    int_t* shift_B = hthread_malloc(0, sizeof(int_t), HT_MEM_RW);
    *shift_B = Ng*sizeof(double);
    obj obj_shift_B = obj_import(res, shift_B, NULL, obj_prop_ready);

    int_t* shift_C = hthread_malloc(0, sizeof(int_t), HT_MEM_RW);
    *shift_C = Ng*sizeof(double);
    obj obj_shift_C = obj_import(res, shift_C, NULL, obj_prop_ready);
    ...
    ddq_add_inputs(generator_Agsm_2, 1, obj_iter_Agsm_1); //0x400101280
    // ddq_add_inputs(generator_Agsm, 2, symbol);
    ...
    ddq_add_inputs(Agsm2Asm, 0, obj_Agsm); //0x400100938 0x400102058
    ...
    ddq_add_inputs(generator_Bgsm, 1, obj_iter_B); //0x4001011d0
    ...
    ddq_add_inputs(generator_C, 1, obj_iter_start_C);
    ...
    ddq_add_inputs(op_matmul, 1, obj_Bam); //0x4001009e8
    ddq_add_inputs(op_matmul, 2, obj_Cam); //0x400100a40
