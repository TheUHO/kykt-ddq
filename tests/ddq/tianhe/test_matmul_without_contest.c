...
ddq_ring matmul_ring(int_t* size_array, double* imat0, double* imat1, double* omat, int_t* res_array, int_t res_idx){
	...
	ddq_ring ring = ddq_new(NULL, 0,0,1024*5);
	obj size_array_dup = obj_import(ring, size_array, NULL, obj_prop_ready, 0);
	obj res_array_dup = obj_import(ring, res_array, NULL, obj_prop_ready, 0);
	obj imat0_dup = obj_import(ring, imat0, NULL, obj_prop_ready, 0);
	obj imat1_dup = obj_import(ring, imat1, NULL, obj_prop_ready, 0);
	obj omat_dup = obj_import(ring, omat, NULL, obj_prop_ready, 0);

	obj imat0_blk = obj_new(ring, ptr_new, raw_del, obj_prop_consumable, 0);
	obj imat1_blk = obj_new(ring, ptr_new, raw_del, obj_prop_consumable, 0);
	obj omat_blk = obj_new(ring, ptr_new, raw_del, obj_prop_consumable, 0);
	obj imat0_ldm = obj_new(ring, mat_new, raw_del, obj_prop_consumable, 0);
	obj imat1_ldm = obj_new(ring, mat_new, raw_del, obj_prop_consumable, 0);
	obj omat_ldm = obj_new(ring, mat_new, raw_del, obj_prop_consumable, 0);
	obj dma_chl0 = obj_new(ring, int_new, raw_del, obj_prop_consumable, 0);
	obj dma_chl1 = obj_new(ring, int_new, raw_del, obj_prop_consumable, 0);
	obj dma_symbol0 = obj_new(ring, int_new, raw_del, obj_prop_consumable, 0);
	obj dma_symbol1 = obj_new(ring, int_new, raw_del, obj_prop_consumable, 0);
	
	int_t* res_idx_ptr = hthread_malloc(0, sizeof(int_t), HT_MEM_RW);
	*res_idx_ptr = res_idx;
	obj res_idx_obj = obj_import(ring, res_idx_ptr, NULL, obj_prop_ready, 0);

	obj idx = obj_new(ring, long_new, raw_del, 0, 0);
	ddq_op initial = ddq_spawn(ring, processor_direct, 0, 1);
	ddq_add_f(initial, obj_import(ring, load_tianhe("op_initial"), NULL, obj_prop_ready, 0));
	ddq_add_outputs(initial, 0, idx);

	ddq_op op_iter_to_ptr = ddq_spawn(ring, processor_direct, 7, 3);
	ddq_add_f(op_iter_to_ptr, obj_import(ring, load_tianhe("op_iter_to_ptr"), NULL, obj_prop_ready, 0));
	...
	ddq_add_f(cluster2sLDM_00, obj_import(ring, load_tianhe("op_cluster2sLDM_Async"), NULL, obj_prop_ready, 0));
	ddq_add_inputs(cluster2sLDM_00, 0, imat0_blk);
	ddq_add_inputs(cluster2sLDM_00, 1, obj_import(ring, BLOCK_ROW_SIZE * BLOCK_REDUCE_SIZE  * sizeof(double), NULL, obj_prop_ready, 0));
	...
	ddq_add_f(cluster2sLDM_01, obj_import(ring, load_tianhe("op_cluster2sLDM_Async"), NULL, obj_prop_ready, 0));
	ddq_add_inputs(cluster2sLDM_01, 0, imat1_blk);
	ddq_add_inputs(cluster2sLDM_01, 1, obj_import(ring, BLOCK_COL_SIZE * BLOCK_REDUCE_SIZE  * sizeof(double), NULL, obj_prop_ready, 0));
	...
	ddq_add_f(dma_query_00, obj_import(ring, load_tianhe("op_dma_query"), NULL, obj_prop_ready, 0));
	...
	ddq_add_f(dma_query_01, obj_import(ring, load_tianhe("op_dma_query"), NULL, obj_prop_ready, 0));
	...
	ddq_add_f(matmul, obj_import(ring, load_tianhe("op_matmul"), NULL, obj_prop_ready, 0));
	...
	ddq_add_f(op_sLDM2cluster0, obj_import(ring, load_tianhe("op_sLDM2cluster"), NULL, obj_prop_ready, 0));
	ddq_add_inputs(op_sLDM2cluster0, 0, omat_ldm);
	ddq_add_inputs(op_sLDM2cluster0, 1, obj_import(ring, BLOCK_COL_SIZE * BLOCK_ROW_SIZE * sizeof(double), NULL, obj_prop_ready, 0));
...
void thread_ring(ddq_ring top_ring, obj idx_array_obj, obj finish_iter_idx_obj,  int_t*idx_array, int_t* size_array, double* imat0, double* imat1, double* omat, int_t thread_idx){
	...
	obj f_getMTthread = obj_import(top_ring, load_so_sym(handle_op, "op_getMTthread"), NULL, obj_prop_ready, 0);
	obj f_freeMTthread = obj_import(top_ring, load_so_sym(handle_op, "op_freeMTthread"), NULL, obj_prop_ready, 0);
    obj core_num_obj = obj_import(top_ring, core_num, NULL, obj_prop_ready, 0);
	obj thread = obj_new(top_ring, load_so_sym(handle_type, "new_tianhe_thread"), load_so_sym(handle_type, "del_tianhe_thread"), obj_prop_consumable, 0);
	obj obj_dsp_finish = obj_import(top_ring, NULL, NULL, obj_prop_consumable, 0);

	ddq_op getMTthread = ddq_spawn(top_ring, processor_direct, 2, 1);
	...
	ddq_add_f(op, obj_import(top_ring, dsp_ring, NULL, obj_prop_ready, 0));
...
ddq_ring cpu_ring(double* imat0, double* imat1, double* omat, int_t row_size, int_t col_size, int_t reduce_size){
	...
	*finish_iter_idx = 0;
	...
	ddq_ring ring = ddq_new(NULL, 0,0, 1024*32);

	void* handle_tensor_ops = load_so_open("tensor/host/ops");
    obj op_imat_to_blk = obj_import(ring, load_so_sym(handle_tensor_ops, "op_imat_to_blk"), NULL, obj_prop_ready, 0);
    
	obj imat0_obj = obj_import(ring, imat0, NULL, obj_prop_ready, 0);
	obj imat1_obj = obj_import(ring, imat1, NULL, obj_prop_ready, 0);
	obj omat_obj = obj_import(ring, omat, NULL, 0, 0);

	obj imat0_blk_obj = obj_import(ring, imat0_blk, NULL, 0, 0);
	obj imat1_blk_obj = obj_import(ring, imat1_blk, NULL, 0, 0);
	obj omat_blk_obj = obj_import(ring, omat_blk, NULL, obj_prop_ready, 0);

	obj size_array_obj = obj_import(ring, size_array, NULL, obj_prop_ready, 0);
	obj idx_array_obj = obj_import(ring, idx_array, NULL, obj_prop_consumable , iter_size);
	printf("idx_array_obj: n_readers:%ld, n_writers:%ld\n", idx_array_obj->n_readers, idx_array_obj->n_writers);
	obj iter_idx_obj = obj_import(ring, iter_idx, NULL, obj_prop_ready, 0);

	obj finish_iter_idx_obj = obj_import(ring, finish_iter_idx, NULL, obj_prop_consumable, 0);
	...
	obj op_iter_blk_raw_obj = obj_import(ring, load_so_sym(handle_tensor_ops, "op_iter_blk_raw"), NULL, obj_prop_ready, 0);
	...
	obj op_iter_finish_obj = obj_import(ring, load_so_sym(handle_tensor_ops, "op_iter_finish"), NULL, obj_prop_ready, 0);
	...
	ddq_add_f(blk_to_omat, obj_import(ring, load_so_sym(handle_tensor_ops, "op_blk_to_omat"), NULL, obj_prop_ready, 0));
